...
{
    return (LV_HOR_RES * width) / 1024;
}

static int pos_height_conversion(int height)
{
    return (LV_VER_RES * height) / 600;
}
...
void dashboard_ui_init()
{
    ...
    if (disp_size == DISP_SMALL) {
        lv_img_set_src(img_bg, LVGL_PATH(bg/normal.png));
    ...
    lv_obj_set_pos(bg_fps, pos_width_conversion(20), pos_height_conversion(30));
    ...
    lv_style_set_arc_image_src(&style_fp, LVGL_PATH(point/normal.png));

    // Create an Arc
    arc = lv_arc_create(lv_scr_act());
    if (disp_size == DISP_SMALL) {
        lv_obj_set_size(arc, 243, 243);
