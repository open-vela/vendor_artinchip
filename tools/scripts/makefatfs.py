...
def gen_fatfs(tooldir, srcdir, outimg, volab, imgsiz, sector_siz, cluster):
    ...
    def get_tool_path(tool_name):
        if tooldir:
            return os.path.join(tooldir, tool_name)
        return tool_name
    ...
    if platform.system() == 'Linux':
        ...
        mformat = '{} -i {}'.format(get_tool_path('mformat'), outimg)
        ...
        if os.path.exists(srcdir) is True:
            mcopy = '{} -i {} -s {}// ::/'.format(get_tool_path('mcopy'), outimg, srcdir)
            run_cmd(mcopy)

        # gen sparse format
        img2simg = '{} {} {}.sparse 1024'.format(get_tool_path('img2simg'), outimg, outimg)
        run_cmd(img2simg)
        ...
        truncate = '{} {} {}'.format(get_tool_path('truncate.exe'), outimg, imgsiz)
        run_cmd(truncate)

        mformat = '{} -i {}'.format(get_tool_path('mformat.exe'), outimg)
        mformat += ' -M {} -T {} -c {}'.format(sector_siz, sector_cnt, cluster)
        if volab != 'default':
            mformat += ' -v {}'.format(volab)
        ...
        if os.path.exists(srcdir) is True:
            mcopy = '{} -i {} -s {}\\\\ ::/'.format(get_tool_path('mcopy.exe'), outimg, srcdir)
            run_cmd(mcopy)

        # gen sparse format
        img2simg = '{} {} {}.sparse 1024'.format(get_tool_path('img2simg.exe'), outimg, outimg)
        run_cmd(img2simg)
