...
static const sfud_qspi_flash_qe_info qspi_flash_qe_info_table[] =
    SFUD_FLASH_QE_INFO_TABLE;
...
int spi_nor_quad_enable(sfud_flash *flash) {
  ...
  /* get wr_reg_status, rd_reg_status and qe_bit, If don't found, the default is
   * compatible sr2_bit1 or SFDP */
  for (int i = 0;
    ...
    if ((qspi_flash_qe_info_table[i].mf_id == flash->chip.mf_id) &&
        ...
        (qspi_flash_qe_info_table[i].capacity_id == flash->chip.capacity_id)) {
