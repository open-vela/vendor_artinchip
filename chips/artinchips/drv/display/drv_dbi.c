...
static const struct aic_dbi_info dbi_info[] = {
    ...
    {
        .type = I8080,
        .format = I8080_RGB666_16BIT_2CYCLE,
        .bpp = 16,
    },
    {
        .type = I8080,
    ...
    {
        .type = I8080,
        .format = I8080_RGB666_18BIT,
        .bpp = 18,
    },
    {
        .type = I8080,
        .format = I8080_RGB888_24BIT,
        .bpp = 24,
    },
    {
        .type = SPI,
    ...
    {
        .type = SPI,
        .format = SPI_3LINE_RGB666,
        .bpp = 18,
    },
    {
        .type = SPI,
        .format = SPI_3LINE_RGB888,
        .bpp = 24,
    },
    {
        .type = SPI,
...
static void aic_dbi_spi_cfg(void) {
  ...
  if (dbi->first_line || dbi->other_line)
    spi_cmd_ctl(comp->regs, dbi->first_line, dbi->other_line);

  if (spi) {
    qspi_code_cfg(comp->regs, spi->code[0], spi->code[1], spi->code[2]);
    qspi_mode_cfg(comp->regs, spi->code1_cfg, spi->vbp_num, spi->qspi_mode);
  }
}
...
static int aic_dbi_enable(void) {
  ...
  reg_set_bits(comp->regs + DBI_CTL, DBI_CTL_TYPE_MASK,
               DBI_CTL_TYPE(dbi->type));

  switch (dbi->type) {
  case I8080:
    reg_set_bits(comp->regs + DBI_CTL, DBI_CTL_I8080_TYPE_MASK,
                 DBI_CTL_I8080_TYPE(dbi->format));
    aic_dbi_i8080_cfg();
    break;
  case SPI:
    reg_set_bits(comp->regs + DBI_CTL, DBI_CTL_SPI_TYPE_MASK,
                 DBI_CTL_SPI_TYPE(dbi->format / SPI_MODE_NUM));
    reg_set_bits(comp->regs + DBI_CTL, DBI_CTL_SPI_FORMAT_MASK,
...
static int aic_dbi_probe(void) {
  ...
  comp = (struct aic_dbi_comp *)kmm_zalloc(sizeof(*comp));
  if (!comp) {
