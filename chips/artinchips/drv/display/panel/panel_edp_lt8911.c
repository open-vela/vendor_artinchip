...
#define LT8911_EDP_DATA_SEQUENCE                                               \
...
static void lt8911_i2c_multi_reg_write(struct rt_i2c_bus_device *i2c,
...
static void lt8911ex_tx_swingpreset(void) {
  const struct reg_sequence seq[] = {
      {0xFF, 0x82}, {0x22, 0x82}, {0x23, 0x00}, {0x24, 0x80}, {0x25, 0x00},
      {0x26, 0x82}, {0x27, 0x00}, {0x28, 0x80}, {0x29, 0x00},
  };
...
static bool lt8911_lvds_signal_check(struct aic_panel *panel) {
  ...
  LT8911_DEV_DEBUG("hfp, hs, hbp, hact, htotal = %d, %d, %d, %d, %d\n", h_fp,
                   h_sync, h_bp, h_active, h_total);

  LT8911_DEV_DEBUG("vfp, vs, vbp, vact, vtotal = %d, %d, %d, %d, %d\n", v_fp,
                   v_sync, v_bp, v_active, v_total);
#endif
