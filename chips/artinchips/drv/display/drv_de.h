...
#define AICFB_FORMAT MPP_FMT_RGB_888
...
static inline bool need_use_hsbc(struct aic_de_comp *comp,
                                 struct aicfb_disp_prop *disp_prop) {
  if (disp_prop->bright != 50 || disp_prop->contrast != 50 ||
      disp_prop->saturation != 50 || disp_prop->hue != 50)
    return true;
...
static inline int de_update_csc(struct aic_de_comp *comp,
  ...
  if (need_use_hsbc(comp, disp_prop)) {
    ...
    de_set_hsbc_with_csc_coefs(comp->regs, color_space, bright, contrast,
                               saturation, hue);
  } else {
