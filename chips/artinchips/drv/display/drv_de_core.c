...
static int aic_de_handler(int irq, void *ctx, void *arg) {
  struct aic_de_comp *comp = (struct aic_de_comp *)arg;
  u32 status;

  if (!comp) {
    pr_err("DE interrupt handler: comp is NULL\n");
    return -EINVAL;
  }
...
#else
  u32 total_num =
      comp->config->layer_num->vi_num + comp->config->layer_num->ui_num;
...
static void aic_de_calc_config(const struct display_timing *timing) {
  ...
  u32 vtotal = timing->vactive + timing->vfront_porch + timing->vback_porch +
               timing->vsync_len;
  u32 htotal = timing->hactive + timing->hfront_porch + timing->hback_porch +
               timing->hsync_len;
...
static const struct de_fmt_info de_formats[] = {
    ...
    {
        .fmt = MPP_FMT_ABGR_8888,
        .bpp = 32,
    },
    {
        .fmt = MPP_FMT_RGBA_8888,
        .bpp = 32,
    },
    {
        .fmt = MPP_FMT_BGRA_8888,
        .bpp = 32,
    },
    {
    ...
    {
        .fmt = MPP_FMT_XBGR_8888,
        .bpp = 32,
    },
    {
        .fmt = MPP_FMT_RGBX_8888,
        .bpp = 32,
    },
    {
        .fmt = MPP_FMT_BGRX_8888,
        .bpp = 32,
    },
    {
        .fmt = MPP_FMT_RGB_888,
    ...
    {
        .fmt = MPP_FMT_ARGB_1555,
        .bpp = 16,
    },
    {
        .fmt = MPP_FMT_ABGR_1555,
        .bpp = 16,
    },
    {
        .fmt = MPP_FMT_RGBA_5551,
        .bpp = 16,
    },
    {
        .fmt = MPP_FMT_BGRA_5551,
        .bpp = 16,
    },
    {
        .fmt = MPP_FMT_RGB_565,
    ...
    {
        .fmt = MPP_FMT_ARGB_4444,
        .bpp = 16,
    },
    {
        .fmt = MPP_FMT_ABGR_4444,
        .bpp = 16,
    },
    {
        .fmt = MPP_FMT_RGBA_4444,
        .bpp = 16,
    },
    {
        .fmt = MPP_FMT_BGRA_4444,
        .bpp = 16,
    },
};

static int aic_de_set_mode(struct aic_panel *panel, struct panel_desc *desc,
                           u32 output_bpp, u32 dither_en) {
  struct aic_de_comp *comp = aic_de_request_drvdata();
  u32 source_bpp;
  int i;
...
static int aic_de_timing_enable(void) {
  ...
  de_config_timing(comp->regs, active_w, active_h, hfp, hbp, vfp, vbp, hsync,
                   vsync, h_pol, v_pol);
  ...
  if (comp->dither.enable)
    de_set_dither(comp->regs, comp->dither.red_bitdepth,
  ...
  de_ui_alpha_blending_enable(comp->regs, comp->alpha[1].value,
                              comp->alpha[1].mode, comp->alpha[1].enable);

  irq_attach(DE_IRQn + RISCV_IRQ_ASYNC, (xcpt_t)aic_de_handler, comp);
  up_enable_irq(DE_IRQn + RISCV_IRQ_ASYNC);
...
static int aic_de_wait_for_vsync(void) {
  ...
  ret = nxsem_wait(&comp->vsync_sem);
  if (ret < 0) {
    pr_err("DE wait vsync irq failed, ret: %d\n", ret);
    return ret;
  }
  return 0;
}
...
static int aic_de_update_alpha_config(struct aicfb_alpha_config *alpha) {
  ...
  de_ui_alpha_blending_enable(comp->regs, alpha->value, alpha->mode,
                              alpha->enable);
  de_config_update_enable(comp->regs, 1);
  memcpy(&comp->alpha[alpha->layer_id], alpha,
         sizeof(struct aicfb_alpha_config));
...
static int aic_de_get_layer_config(struct aicfb_layer_data *layer_data) {
  ...
  memcpy(layer_data, &comp->layers[index], sizeof(struct aicfb_layer_data));
...
static int update_one_layer_config(struct aic_de_comp *comp,
  ...
  if (layer_data->enable == 0) {
    ...
    if (is_ui_layer(comp, layer_data->layer_id)) {
      ui_rect_disable(comp, layer_data->layer_id, layer_data->rect_id);
    } else {
  ...
  if (is_ui_layer(comp, layer_data->layer_id)) {
    ...
    if (ret != 0)
      ...
      memcpy(&comp->layers[index], layer_data, sizeof(struct aicfb_layer_data));
  } else {
...
static int aic_de_probe(void) {
  ...
  comp = (struct aic_de_comp *)kmm_zalloc(sizeof(struct aic_de_comp));
  if (!comp) {
