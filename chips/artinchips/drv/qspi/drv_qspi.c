 ...
 *
 * Authors: xinming.chen <xinming.chen@artinchip.com>
 */
...
struct aic_spi_priv_s {
  struct spi_dev_s spi_dev;
  //   struct hal_spi_master spim;
  struct aic_qspi_bus *bus;
...
static struct aic_qspi_bus qspi_bus_arr[] = {
    {
        ...
        .idx = 0,
        .clk_id = CLK_QSPI0,
        .clk_in_hz = 0, // AIC_DEV_QSPI0_MAX_SRC_FREQ_HZ,
        .bus_hz = 0,    // AIC_QSPI0_DEVICE_SPINOR_FREQ,
        .dma_port_id = DMA_ID_SPI0,
        .irq_num = QSPI0_IRQn,
        .dl_width = 0, // AIC_QSPI0_BUS_WIDTH,
#if defined(AIC_QSPI_MULTIPLE_CS_NUM)
        .cs_num = AIC_QSPI0_CS_NUM,
#endif
        .rxd_dylmode = 0, // AIC_DEV_QSPI0_DELAY_MODE,
...
static struct aic_spi_priv_s g_aic_spi0_priv = {
    .spi_dev = {.ops = &g_qspi_ops},
    .bus = &qspi_bus_arr[0],
    .mode = SPIDEV_MODE0,
};

int spi_write_read(struct aic_qspi_bus *qspi, const uint8_t *write_buf,
...
struct aic_qspi_bus *qspi_probe(u32 spi_bus) {
  ...
  spi_register(&g_aic_spi0_priv.spi_dev, 0);
#endif
  return qspi;
}

void hw_qspi_bus_init(int idx) { qspi_probe(idx); }
