...
u32 mtd_get_device_count(void) {
  ...
  list_for_each(pos, &mtd_dev_list) { cnt++; }
  return cnt;
}
...
int mtd_read_oob(struct mtd_dev *mtd, u32 offset, uint8_t *data, u32 len,
                 uint8_t *spare_data, u32 spare_len) {
  if (mtd && mtd->ops.read_oob)
    return mtd->ops.read_oob(mtd, offset, data, len, spare_data, spare_len);
  return -1;
}

int mtd_write_oob(struct mtd_dev *mtd, u32 offset, uint8_t *data, u32 len,
                  uint8_t *spare_data, u32 spare_len) {
...
int mtd_map_oob_user_region(struct mtd_dev *mtd, u8 *oobbuf, u8 *buf, int start,
                            int nbytes) {
...
int mtd_unmap_oob_user_region(struct mtd_dev *mtd, u8 *dst, u8 *src, int start,
                              int nbytes) {
