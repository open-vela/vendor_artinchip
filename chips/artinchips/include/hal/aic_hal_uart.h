...
#define FRC_TX_FIFO_SET(x)                                                     \
  (x << 4) /* 0 is null, 1 is 2byte, 2 is 1/4 fifo, 3 is 1/2 fifo */
#define FRC_RX_FIFO_SET(x)                                                     \
  (x << 6) /* 0 is 1byte, 1 is 1/4 fifo, 2 is 1/2 fifo, 3 is full*/
...
#define AIC_RXCTL_DLAB_ACEN                                                    \
  0X04 /* Receiver divisor latch access bit auto clear enable */
...
typedef enum {
  ...
  USART_MODE_RS232_UNAUTO_FLOW_CTRL =
      4,                             ///< RS232 hardware unauto flow conctrol
  USART_MODE_RS232_SW_FLOW_CTRL = 5, ///< RS232 software flow conctrol
  USART_MODE_RS232_SW_HW_FLOW_CTRL =
      6, ///< RS232 software and hardware flow conctrol
 ...
 * -----*/
typedef enum {
  USART_CPOL0 = 0, ///< CPOL = 0 (default). data are captured on rising edge
                   ///< (low->high transition)
  USART_CPOL1      ///< CPOL = 1. data are captured on falling edge (high->lowh
                   ///< transition)
} usart_cpol_e;
...
typedef enum { USART_FLUSH_WRITE, USART_FLUSH_READ } usart_flush_type_e;
...
typedef struct {
  ...
  uint32_t
      tx_underflow : 1; ///< Transmit data underflow detected (cleared on start
                        ///< of next send operation)(Synchronous Slave)
  uint32_t rx_overflow : 1; ///< Receive data overflow detected (cleared on
                            ///< start of next receive operation)
  uint32_t rx_break : 1;    ///< Break detected on receive (cleared on start of
                            ///< next receive operation)
  uint32_t rx_framing_error : 1; ///< Framing error detected on receive (cleared
                                 ///< on start of next receive operation)
  uint32_t rx_parity_error : 1;  ///< Parity error detected on receive (cleared
                                 ///< on start of next receive operation)
...
typedef enum {
  USART_EVENT_SEND_COMPLETE =
      0, ///< Send completed; however USART may still transmit data
  ...
  USART_EVENT_TX_UNDERFLOW =
      4, ///< Transmit data not available (Synchronous Slave)
  ...
  USART_EVENT_RECEIVED = 14,        ///< Data Received, only in usart fifo, call
                                    ///< receive()/transfer() get the data
} usart_event_e;

typedef void (*usart_event_cb_t)(
    int32_t idx,
    usart_event_e
        event); ///< Pointer to \ref usart_event_cb_t : USART Event call back.
...
/**
  ...
  usart index \param[in]   cb_event  event call back function \ref
  usart_event_cb_t \return      return usart handle if success
*/
usart_handle_t hal_usart_initialize(int32_t idx, usart_event_cb_t cb_event,
                                    void *handler);

/**
  \brief       De-initialize USART Interface. stops operation and releases the
...
/**
  \brief       Start sending data to USART transmitter,(received data is
  ignored). This function is non-blocking,\ref usart_event_e is signaled when
  operation completes or error happens. \ref hal_usart_get_status can get
  operation status. \param[in]   handle  usart handle to operate. \param[in]
  data  Pointer to buffer with data to send to USART transmitter. data_type is :
  uint8_t for 5..8 data bits, uint16_t for 9 data bits \param[in]   num   Number
  of data items to send \return      error code
*/
...
/**
  \brief       Start receiving data from USART receiver. \n
               This function is non-blocking,\ref usart_event_e is signaled when
  operation completes or error happens. \ref hal_usart_get_status can get
  operation status. \param[in]   handle  usart handle to operate. \param[out]
  data  Pointer to buffer for data to receive from USART receiver.data_type is :
  uint8_t for 5..8 data bits, uint16_t for 9 data bits \param[in]   num   Number
  of data items to receive \return      error code
*/
...
/**
  \brief       Start synchronously sends data to the USART transmitter and
  receives data from the USART receiver. used in synchronous mode This function
  is non-blocking,\ref usart_event_e is signaled when operation completes or
  error happens. \ref hal_usart_get_status can get operation status. \param[in]
  handle  usart handle to operate. \param[in]   data_out  Pointer to buffer with
  data to send to USART transmitter.data_type is : uint8_t for 5..8 data bits,
  uint16_t for 9 data bits \param[out]  data_in   Pointer to buffer for data to
  receive from USART receiver.data_type is : uint8_t for 5..8 data bits,
  uint16_t for 9 data bits \param[in]   num       Number of data items to
  transfer \return      error code
*/
...
/**
  ...
  \param[in]   handle  usart handle to operate.
  \param[in]   enable  1- Enable continuous Break transmission,0 - disable
  continuous Break transmission \return      error code
*/
...
int32_t hal_usart_config_rs485(usart_handle_t handle, bool rs485,
                               bool compactio);
