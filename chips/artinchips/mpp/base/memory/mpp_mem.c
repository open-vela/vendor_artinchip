...
extern void *mpp_dec_malloc_oom(size_t size);
extern void *mpp_dec_memalign_oom(size_t size, size_t align);
extern void  mpp_dec_free_oom(void *ptr);
#endif

void *aicos_malloc_oom(size_t size)
{
#ifdef CONFIG_LV_USE_MPP_DEC
    return mpp_dec_malloc_oom(size);
#else
...
{
#ifdef CONFIG_LV_USE_MPP_DEC
    return mpp_dec_memalign_oom(size, align);
#else
...
{
    unsigned int addr = (unsigned long)aicos_memalign_oom(size, 1024);
    if (addr == 0) {
...
void mpp_phy_free(unsigned int addr)
{
    ...
    aicos_free_oom( (void*)(unsigned long)addr);
