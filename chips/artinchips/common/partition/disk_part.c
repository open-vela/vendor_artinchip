...
static int validate_gpt_header(gpt_header *gpt_h, u64 lba, u64 lastlba) {
  ...
  pr_debug("GPT: first_usable_lba: %llX last_usable_lba: %llX last lba: %llX n",
...
struct aic_partition *aic_disk_get_gpt_parts(struct blk_desc *dev_desc) {
  ...
  for (i = 0; i < (gpt_head->num_partition_entries); i++) {
    ...
    n->size =
        (gpt_pte[i].ending_lba + 1 - gpt_pte[i].starting_lba) * dev_desc->blksz;
...
int gpt_verify_partitions(struct blk_desc *dev_desc,
  ...
  while (part) {
    ...
    pr_debug("%s: part: %2d name - GPT: %16s, ENV: %16s ", __func__, i, efi_str,
             part->name);
    ...
    pr_debug("size(LBA) - GPT: %8llu, ENV: %8llu ",
             (unsigned long long)gpt_part_size, (unsigned long long)part->size);
...
int find_valid_gpt(struct blk_desc *dev_desc, gpt_header *gpt_head,
  ...
  if (r != 1) {
    ...
    if (is_gpt_valid(dev_desc, (dev_desc->lba_count - 1), gpt_head, pgpt_pte) !=
        1) {
...
gpt_entry *alloc_read_gpt_entries(struct blk_desc *dev_desc,
  ...
  count =
      (pgpt_head->num_partition_entries) * (pgpt_head->sizeof_partition_entry);
