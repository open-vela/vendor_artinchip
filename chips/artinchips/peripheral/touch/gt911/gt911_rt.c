...
static rt_err_t gt911_get_info(struct rt_i2c_client *dev,
  ...
  rt_device_control((rt_device_t)touch, RT_TOUCH_CTRL_GET_DYNAMIC_ROTATE,
                    &angle);
...
static rt_size_t gt911_read_point(struct rt_touch_device *touch, void *buf,
  ...
  if (touch_num) /* point down */
  {
    ...
    rt_device_control((rt_device_t)touch, RT_TOUCH_CTRL_GET_DYNAMIC_FLAG,
                      &dynamic_flag);
    rt_device_control((rt_device_t)touch, RT_TOUCH_CTRL_GET_OSD_FLAG,
                      &osd_flag);
    ...
    for (read_index = 0; read_index < touch_num; read_index++) {
      ...
      // Step 1: Perform coordinate transformations only when osd mode is
      // disabled
      if (!osd_flag) {
...
static rt_err_t gt911_modify_firmware(struct rt_touch_device *touch, int cmd,
                                      void *arg) {
  rt_uint8_t buf[4] = {0};
  rt_uint8_t i = 0;
  rt_uint8_t *config;
...
static rt_err_t gt911_control(struct rt_touch_device *touch, int cmd,
                              void *arg) {
...
static int rt_hw_gt911_init(const char *name, struct rt_touch_config *cfg) {
  ...
  if (RT_EOK != rt_hw_touch_register(touch_device, name, RT_DEVICE_FLAG_INT_RX,
                                     RT_NULL)) {
