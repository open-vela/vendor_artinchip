...
#define to_clk_cpu_mod(_hw) container_of(_hw, struct aic_clk_cpu_cfg, comm)
...
static int clk_cpu_enable_and_deassert_rst(struct aic_clk_comm_cfg *comm_cfg) {
...
static void clk_cpu_disable_and_assert_rst(struct aic_clk_comm_cfg *comm_cfg) {
  struct aic_clk_cpu_cfg *mod = to_clk_cpu_mod(comm_cfg);
  u32 val;
...
static void try_best_divider(u32 rate, u32 parent_rate, u32 max_div0,
                             u32 *div0) {
...
static unsigned int clk_cpu_mod_get_parent(struct aic_clk_comm_cfg *comm_cfg) {
  struct aic_clk_cpu_cfg *mod = to_clk_cpu_mod(comm_cfg);
  u32 index = (readl(cmu_reg(mod->offset_reg)) >> mod->mux_bit) & mod->mux_mask;
