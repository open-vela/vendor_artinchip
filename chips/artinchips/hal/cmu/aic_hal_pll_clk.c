...
static void clk_vco_select(struct aic_clk_pll_cfg *pll, unsigned long *min,
                           unsigned long *max) {
...
static inline void clk_pll_bypass(struct aic_clk_pll_cfg *pll,
                                  unsigned int bypass) {
...
static void clk_pll_enable_sdm(struct aic_clk_pll_cfg *pll,
                               unsigned long parent_rate, u32 factor_n) {
  ...
  if (ppm_max < PLL_SDM_SPREAD_PPM)
    ...
    sdm_amp = PLL_SDM_AMP_MAX - PLL_SDM_SPREAD_PPM * PLL_SDM_AMP_MAX / ppm_max;
  ...
  reg_val = (1UL << PLL_SDM_EN_BIT) | (2 << PLL_SDM_MODE_BIT) |
            (sdm_step << PLL_SDM_STEP_BIT) | (3 << PLL_SDM_FREQ_BIT) |
...
static unsigned long clk_pll_recalc_rate(struct aic_clk_comm_cfg *comm_cfg,
  ...
  factor_n =
      (readl(cmu_reg(pll->offset_gen)) >> PLL_FACTORN_BIT) & PLL_FACTORN_MASK;
  factor_m =
      (readl(cmu_reg(pll->offset_gen)) >> PLL_FACTORM_BIT) & PLL_FACTORM_MASK;
  factor_p =
      (readl(cmu_reg(pll->offset_gen)) >> PLL_FACTORP_BIT) & PLL_FACTORP_MASK;
  ...
  if (pll->type == AIC_PLL_INT || !fra_en) {
    ...
    rate_int = parent_rate / (factor_p + 1) * (factor_n + 1) / (factor_m + 1);
...
static int clk_pll_set_rate(struct aic_clk_comm_cfg *comm_cfg,
  ...
  reg_val |= (factor_m_en << PLL_FACTORM_EN_BIT) |
             (factor_n << PLL_FACTORN_BIT) | (factor_m << PLL_FACTORM_BIT) |
  ...
  if (pll->type == AIC_PLL_FRA) {
    val =
        rate % (parent_rate * (factor_n + 1) / (factor_m + 1) / (factor_p + 1));
    fra_en = val ? 1 : 0;
    if (fra_en) {
      fra_in = val * (factor_p + 1) * (factor_m + 1) * PLL_FRAC_DIV_MASK;
...
void hal_clk_pll_lowpower(void) {
  ...
  if (xtal_en)
    *(volatile uint32_t *)(CMU_BASE + PLL_IN_REG) &=
        ~((0x7U << 29) | (0x1U << 1));
  else
    *(volatile uint32_t *)(CMU_BASE + PLL_IN_REG) &=
        ~((0x7U << 29) | (0x1U << 28));
