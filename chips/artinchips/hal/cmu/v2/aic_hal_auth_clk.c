...
#define to_clk_auth(_hw) container_of(_hw, struct aic_clk_auth_cfg, comm)
...
static int clk_auth_write(struct aic_clk_auth_cfg *mod, u32 val) {
  ...
  if (cmu_auth_request(mod->offset_wr_auth_reg, mod->key_code, mod->key_bit,
                       BIT(mod->wr_auth_bit), mod->offset_reg))
    return -EIO;
