...
/* GE interrupt handler - not called at runtime (VIC disabled).
 * Kept for registration purposes only. Actual completion detection
 * is done by polling ge_read_status() in ge_start_and_wait().
 */
static irqreturn_t aic_ge_handler(int irq, void *ctx, void *arg)
{
    struct aic_ge_data *data = (struct aic_ge_data *)arg;
...
int hal_ge_init(void)
{
    ...
    if (!data->wait) {
        hal_log_err("failed to create event\n");
    ...
    if (!data->lock) {
        hal_log_err("failed to create mutex\n");
        aicos_event_delete(data->wait);
        aicos_free(GE_DEFAULT, data);
        return -1;
    }

#ifdef CONFIG_AIC_GE_DITHER
    ...
    /* Set g_data and enable clock BEFORE interrupt, so ISR can access safely */
    g_data = data;
    ...
    /* TEST: _xtheade enabled, re-test GE ISR */
    aicos_request_irq(GE_IRQn, aic_ge_handler, 0, NULL, (void *)data);
    aicos_irq_enable(GE_IRQn);
