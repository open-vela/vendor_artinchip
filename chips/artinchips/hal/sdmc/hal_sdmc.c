...
void hal_sdmc_idma_prepare(struct aic_sdmc_host *host, u32 blksize, u32 blks,
  ...
  aicos_dcache_clean_invalid_range(
      (ulong *)data_start, roundup(data_end - data_start, ARCH_DMA_MINALIGN));
...
int hal_sdmc_data_tx(struct aic_sdmc_host *host, u32 *buf, u32 size) {
  u32 i, len = 0;
  u32 fifo_depth =
      (((host->fifoth_val & RX_WMARK_MASK) >> RX_WMARK_SHIFT) + 1) * 2;
...
void aic_sdmc_set_ext_clk_mux(struct aic_sdmc_host *host, u32 mux) {
  ...
  switch (mux) {
  case 1:
    temp |= (u32)SDMC_DLYCTRL_EXT_CLK_MUX_1 << SDMC_DLYCTRL_EXT_CLK_MUX_SHIFT;
    break;
  case 2:
