...
int bounce_buffer_start_extalign(
    struct bounce_buffer *state, void *data, size_t len, unsigned int flags,
    size_t alignment, int (*addr_is_aligned)(struct bounce_buffer *state)) {
  ...
  if (!addr_is_aligned(state)) {
    state->bounce_buffer = malloc(state->len_aligned);
...
int bounce_buffer_start(struct bounce_buffer *state, void *data, size_t len,
                        unsigned int flags) {
  return bounce_buffer_start_extalign(state, data, len, flags, BOUNCE_BUF_ALIGN,
