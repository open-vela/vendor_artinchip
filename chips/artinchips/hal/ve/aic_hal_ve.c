...
static void hal_ve_power_on(void)
{
    hal_ve_clk_enable();
}

static void hal_ve_power_off(void)
{
    hal_ve_clk_disable();
}

static irqreturn_t hal_ve_handle(int irq, void *ctx, void *arg)
{
    u32 status;
    (void)ctx;
    (void)arg;
...
int hal_ve_probe(void)
{
    ...
    nxsem_init(&client.wait_sem, 0, 0);
    aicos_request_irq(VE_IRQn, hal_ve_handle, 0, NULL, (void*)&client);
    aicos_irq_enable(VE_IRQn);
    hal_log_debug("++++ aich_ve_probe, client: %p", &client);

#ifdef CONFIG_AIC_VE_DRV_V40
...
int hal_ve_close(struct aic_ve_client *clt)
{
    hal_ve_power_off();
    return 0;
}
...
int hal_ve_close_with_wait(void)
{
    ...
    if (client.is_busy) {
        int ticks = MSEC2TICK(VE_CLOSE_TIMEOUT_MS);
        if (nxsem_tickwait(&client.wait_sem, ticks) < 0) {
...
int hal_ve_control(struct aic_ve_client *clt, int cmd, void *arg)
{
    switch(cmd) {
    case IOC_VE_WAIT: {
        ...
        if (info->wait_time == (int)AICOS_WAIT_FOREVER || info->wait_time < 0) {
            ret = nxsem_wait(&clt->wait_sem);
        } else {
            int ticks = MSEC2TICK(info->wait_time);
            ret = nxsem_tickwait(&clt->wait_sem, ticks);
        }
        if (ret < 0) {
        ...
        info->reg_status = clt->status;
        break;
    }
    case IOC_VE_GET_CLIENT:
        if(aicos_mutex_take(clt->lock, VE_TIMEOUT) < 0) {
