...
u32 hal_wdt_remain(struct aic_wdt *wdt) {
  ...
  u32 rst_thd = readl(WDT_REG_RST_THD(0));

  /* Calculate remaining time in milliseconds */
  u32 remaining_ticks;
  if (val >= rst_thd) {
    /* Counter has overflowed, remaining time is 0 */
    remaining_ticks = 0;
  } else {
    remaining_ticks = rst_thd - val;
  }
  /* Convert ticks to milliseconds using 32kHz clock frequency */
  /* Use more precise calculation: (ticks * 1000) / 32000 */
  u32 remaining_ms = (remaining_ticks * 1000) / 32000;
  return remaining_ms;
}
...
void hal_wdt_enable(u32 enable, u32 dbg_continue) {
  ...
  if (enable) {
    writel(WDT_CNT_EN | (dbg_continue << WDT_DBG_CNT_CONTINUE_SHIFT),
           WDT_REG_CTL);
    return;
  }
...
void hal_wdt_irq_enable(u32 enable) { writel(enable, WDT_REG_IRQ_EN); }

int hal_wdt_irq_sta(void) { return readl(WDT_REG_IRQ_EN); }
...
#ifdef AIC_WDT_DRV_V11
void hal_wdt_rst_type_set(u32 rst) { writel(rst, WDT_REG_RST_SEL); }
...
void hal_wdt_status_show(u32 ch) {
  ...
  printf("In Watchdog V%d.%02d:\n"
         ...
         readl(WDT_REG_CTL) >> WDT_WR_DIS_SHIFT, readl(WDT_REG_IRQ_EN),
         aic_wdt_cur_id(), ch, readl(WDT_REG_CNT),
#ifdef AIC_WDT_DRV_V11
         readl(WDT_REG_RST_SEL),
#endif
         readl(WDT_REG_CLR_THD(0)), readl(WDT_REG_IRQ_THD(0)),
