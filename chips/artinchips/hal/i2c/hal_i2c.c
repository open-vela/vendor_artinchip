...
/* Keep one switch name across RT-Thread and NuttX builds. */
#if defined(CONFIG_AIC_I2C_INTERRUPT_MODE) && !defined(AIC_I2C_INTERRUPT_MODE)
#define AIC_I2C_INTERRUPT_MODE 1
#endif
...
static int32_t hal_i2c_wait_transmit(aic_i2c_ctrl *i2c_dev, uint32_t timeout) {
  ...
  do {
    ...
    while ((hal_i2c_get_transmit_fifo_num(i2c_dev) != 0U) && (ret == EOK)) {
